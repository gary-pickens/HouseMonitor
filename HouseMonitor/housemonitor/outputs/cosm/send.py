...
class COSMSend(CosmConfiguration):
    ...
    datastreams = None
    ''' A List of data items to send to COSM. '''
    ...
    def output(self, data={}, http=None):
        ...
        try:
            ...
            json = self.createJSONReport(device, port, data)
            self.report_data(json, data, http=http)
    ...
    def createDataStream(self, device, port, data):
        ...
        if Constants.DataPacket.arrival_time in data:
            ...
            self.logger.debug('arrival_datatime = {}'.format(arrival_datetime))
